...
description: >
  Canonical final whole-package review orchestrator for BITM330 database-book chapters
  in the repo-canonical source model. Use after a chapter has been substantially
  edited and is ready for final package review across stable source files under
  books/database-book/files/source. Reviews chapter index, core concepts, Let's Build,
  Terms Treasury, Review and Reflection, RAT, lab, cover/front matter when included,
  media, figure captions, image storage, image manifests, callouts, author comments,
  term consistency, outline coverage, word count, reports, progress logging, edit
  tracking, validation, and DOCX readiness. Coordinates existing specialized skills
  rather than duplicating or replacing them.
argument-hint: >
  Chapter number, chapter folder, source package path, or mode, e.g. "ch01",
  "ch01 full", "ch01 dry-run", "ch01 media-only", "ch01 companions-only",
  "books/database-book/files/source/chapters/ch01-introduction-to-course",
  "0-cover-image front-matter-only", or "01-acknowlgements language-only".
---
...
`chapter-final-review` is the single canonical final-review skill for BITM330 database-book chapters.
...
```text
.agents/skills/chapter-final-review/SKILL.md
```

Optional agent config if local skill-creator convention requires it:

```text
.agents/skills/chapter-final-review/agents/openai.yaml
```
...
description: Canonical final whole-package review orchestrator for BITM330 database-book chapters.
instructions: |
  Use SKILL.md as the controlling instruction file.
  Treat books/database-book/files/source as canonical.
  Coordinate specialized skills rather than duplicating them.
  Gate all writes behind the Phase 0 plan.
  Gate all media side effects behind the media dry run.
  Do not run chapter-sync, deploy, publish, push, or commit unless the user separately and explicitly requests it.
```
