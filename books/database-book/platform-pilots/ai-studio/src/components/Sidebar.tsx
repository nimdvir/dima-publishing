          ...
          <div className="py-3 border-b border-zinc-200 flex flex-col items-center gap-2.5 bg-zinc-50/50 shrink-0">
            ...
            <button
              ...
              className={cn(
                ...
                currentScope === 'ai_chat' ? "bg-amber-600 text-white border-amber-500" : "bg-white border-zinc-200 text-zinc-500 hover:bg-zinc-102"
              )}
              title="AI Syllabus Study Tutor (Premium)"
            >
              <MessageSquare className="w-4 h-4" />
              {user && !user.hasPaid && (
                <div className="absolute -top-1 -right-1 bg-amber-500 border border-white text-white rounded-full flex items-center justify-center p-0.5 shadow-sm">
        ...
        {isOpen && onBackToCover && (
          <div className="px-4 pt-4 pb-1 shrink-0 flex flex-col gap-2">
            ...
            <button
              ...
              className={cn(
                ...
                currentScope === 'ai_chat'
                  ? "bg-amber-600 text-white border-amber-500 shadow-md shadow-amber-100 animate-pulse"
            ...
            >
              ...
              <span className={cn(
                "text-[8px] px-1.5 py-0.5 rounded font-black font-mono border uppercase tracking-wider flex items-center gap-0.5",
                currentScope === 'ai_chat' 
                ...
                {user && !user.hasPaid && <Lock className="w-2.5 h-2.5 shrink-0" />}
                PRO AI
              </span>
            </button>
          </div>
              ...
              {[
                ...
                if (!isOpen) {
                  return (
                    <button
                      key={ap.id}
                      onClick={() => onSelectAppendix(ap.id)}
                      className={cn(
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className={cn("p-1.5 rounded-lg shrink-0", isActive ? "bg-purple-700" : "bg-purple-50 text-purple-700")}>
                        {user && !user.hasPaid && ap.id === 'playground' ? (
                    ...
                    {!isOpen ? (
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                          {user && !user.hasPaid && chapter.id !== 'ch1' ? (
                            <div className="w-4 h-4 rounded-full bg-amber-50 flex items-center justify-center shrink-0 border border-amber-200 text-amber-600">
                          ...
                          <span className={cn(
                            ...
                            user && !user.hasPaid && chapter.id !== 'ch1' && "text-zinc-400/80 font-normal italic"
                          )}>{chapter.title}</span>
                        </div>
