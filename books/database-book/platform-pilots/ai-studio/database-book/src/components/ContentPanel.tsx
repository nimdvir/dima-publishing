...
export function ContentPanel({
  ...
  const { user, highlights, addHighlight, deleteHighlight, updateHighlightNote, completePayment } = useAuth();

  const [showHighlightsNotebook, setShowHighlightsNotebook] = useState(true);
  const [noteScope, setNoteScope] = useState<'section' | 'chapter'>('section');
  ...
  const activeHighlightsList = useMemo(() => {
    return noteScope === 'section' ? sectionHighlights : chapterHighlights;
  }, [noteScope, sectionHighlights, chapterHighlights]);
      ...
      <div className="prose prose-zinc prose-indigo max-w-none text-zinc-700 leading-relaxed text-sm my-4">
        <Markdown
          ...
          components={{
            ...
            table: ({ children }) => (
              <div className="w-full overflow-x-auto my-4 border border-zinc-200 rounded-xl">
                <table className="w-full text-left border-collapse text-xs md:text-sm text-zinc-650 min-w-[500px]">
                  {children}
                </table>
              </div>
            ),
            th: ({ children }) => <th className="bg-zinc-50 border-b border-zinc-200 px-4 py-2.5 text-[10px] sm:text-xs font-bold text-zinc-700 uppercase tracking-wider">{children}</th>,
            td: ({ children }) => <td className="border-b border-zinc-200 px-4 py-2.5 leading-normal">{children}</td>,
          }}
        >
          {text}
        </Markdown>
              ...
              {chapter.concepts.subsections.map((sub, idx) => (
                ...
                >
                  ...
                  <p className="text-zinc-600 leading-relaxed text-sm">
                    {applyHighlights(sub.content, sectionHighlights)}
                  </p>
                </motion.div>
              ))}
            </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {chapter.terms.map((term, index) => (
                ...
                >
                  ...
                  <span className="text-zinc-500 text-xs leading-relaxed">{applyHighlights(term.definition, sectionHighlights)}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-zinc-50 animate-fadeIn overflow-hidden">
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="w-full lg:w-[420px] h-[240px] lg:h-full bg-white border-b lg:border-b-0 lg:border-r border-zinc-200 overflow-y-auto flex flex-col shrink-0">
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-zinc-50 overflow-hidden animate-fadeIn select-none font-sans">
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="w-full lg:w-[480px] h-[240px] lg:h-full bg-white border-b lg:border-b-0 lg:border-r border-zinc-200 overflow-y-auto p-5 space-y-6 flex flex-col shrink-0">
            <div className="space-y-1">
  ...
  // --- Stripe Payment Activation Gateway Overlay ---
  const handleStripeSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!cardName.trim() || cardNumber.replace(/\s/g, '').length < 16 || cardExpiry.length < 5 || cardCvc.length < 3) {
      setPaywallError("Please fill out all payment fields with correct credit card credentials.");
      return;
    }
    ...
    // Simulate real Stripe payment processing pipeline
    setTimeout(async () => {
      try {
        await completePayment();
        setPaymentSuccess(true);
      } catch (err) {
        setPaywallError("Stripe secure gateway is temporarily unresponsive. Loading fallback local voucher code checkout...");
      } finally {
  ...
  const renderStripePaywall = () => {
    const studentName = user ? user.fullName : "MIS Student";
    const netId = user ? user.netId : "unknown";

    if (paymentSuccess) {
      return (
        <div className="flex-1 flex flex-col justify-center items-center p-8 bg-zinc-50 font-sans text-center animate-scaleUp">
          <div className="max-w-md w-full bg-white border border-zinc-200 rounded-3xl p-8.5 shadow-xl space-y-6">
            <div className="w-16 h-16 rounded-full bg-emerald-100 border border-emerald-250 flex items-center justify-center mx-auto text-emerald-600">
              <CheckCircle2 className="w-9 h-9" />
            </div>
            <div className="space-y-2">
              <span className="text-[10px] uppercase font-extrabold text-emerald-600 tracking-wider bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-100">
                Payment Activated Via Stripe
              </span>
              <h2 className="text-xl font-extrabold text-zinc-900 leading-tight">Semester Registration Complete!</h2>
              <p className="text-xs text-zinc-500 leading-relaxed px-2">
                Thank you! Your account (NetID: <strong>{netId}</strong>) has been fully upgraded to permanent student access status. All syllabus modules, laboratory consoles, and database scores are fully unlocked.
              </p>
            </div>
            ...
            <button
              onClick={() => {
                ...
                // Force a gentle state reset to refresh page content immediately!
              }}
              className="w-full text-center py-3 bg-zinc-900 hover:bg-zinc-805 text-white font-bold rounded-xl text-xs transition-all shadow-md cursor-pointer"
            >
              Enter Classroom Textbook
            </button>
          </div>
        </div>
      );
    ...
    return (
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-zinc-50 font-sans">
        {/* Paywall Header */}
        <div className="border-b border-zinc-150 p-6 bg-white shrink-0 flex items-center justify-between">
          <div>
            <span className="text-[9px] font-black text-rose-500 uppercase tracking-widest leading-none bg-rose-50 px-2 py-0.5 rounded border border-rose-100 flex items-center gap-1 inline-block">
              🕒 TRIAL INTERRUPT OVERLAY
            </span>
            <h1 className="text-base font-black text-zinc-800 pt-1">Resources Locked: Trial Period Ended</h1>
          </div>
          
          <div className="flex items-center gap-1.5 text-xs text-zinc-500 font-bold bg-zinc-100 px-3 py-1 rounded-lg border border-zinc-200">
            {/* Padlock SVG */}
        ...
        {/* Core Paywall Split Screen Grid */}
        <div className="flex-1 overflow-y-auto p-6 md:p-8 flex items-center justify-center">
          <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-12 gap-8 items-stretch">
            ...
            <div className="md:col-span-5 bg-white border border-zinc-200 rounded-3xl p-6.5 flex flex-col justify-between shadow-xs">
              <div className="space-y-5">
                <div>
                  <span className="text-[9px] font-black text-indigo-500 uppercase tracking-wider">Class Registration Bill</span>
                  <h3 className="text-sm font-black text-zinc-800 pt-0.5">Management Information Systems</h3>
                  <p className="text-[11px] text-zinc-400">Dr. Nimrod Dvir's Syllabus</p>
                </div>
                ...
                {/* Benefits List */}
                <div className="space-y-3">
                  <p className="text-[10px] font-extrabold text-zinc-400 uppercase tracking-wider">Lifetime student pack unlocks:</p>
                  
                  {[
                    "16 comprehensive visual lecture chapters",
                    "4 automated SQL interactive database labs",
                    "Interactive sandbox, consoles & testing compilers",
                    "Score grading sync and classroom progress tracker",
                    "Glossary glossary database & vocabulary matching game"
                  ].map((benefit, bIdx) => (
                    <div key={bIdx} className="flex gap-2 text-xs text-zinc-650">
              ...
              <div className="space-y-3.5 mt-8 border-t border-zinc-150 pt-5">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold text-zinc-500">Student Account</span>
                  <span className="text-[11px] font-mono text-zinc-700 bg-zinc-100 px-1.5 py-0.5 rounded border border-zinc-150">{netId}</span>
                </div>
                <div className="flex justify-between items-center bg-indigo-50/50 p-2.5 rounded-xl border border-indigo-100">
                  <span className="text-xs font-black text-zinc-700">One-Time License Activation</span>
                  <span className="text-base font-black text-indigo-700">$25.00 USD</span>
                </div>
                
                {/* Professor instant sandbox pay shortcut */}
                <button
                  onClick={handleInstantBypass}
                  className="w-full text-center py-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 hover:text-indigo-800 border border-indigo-150 rounded-xl text-[10px] font-extrabold uppercase tracking-widest transition-all cursor-pointer"
                  title="Grader / Professor Bypass shortcut"
                >
                  ⚡ Grader Checkout Bypass (Instant Active)
                </button>
              </div>
            </div>

            {/* Right Column: Stripe Interactive checkout card interface */}
            <div className="md:col-span-7 bg-white border border-zinc-200 rounded-3xl p-6.5 flex flex-col justify-between shadow-sm relative">
              <div className="absolute right-6 top-6 flex items-center gap-1">
                {/* Stripe SVG badge */}
                <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest select-none">Secure Payment via</span>
                <span className="text-indigo-600 font-mono text-xs font-black tracking-normal italic select-none">stripe</span>
              </div>

              <div className="space-y-6">
                <div>
                  <h3 className="text-sm font-black text-zinc-800">Secure Direct Activation</h3>
                  <p className="text-[11px] text-zinc-400">Complete activation effortlessly using any VISA, MasterCard, or Amex.</p>
                </div>

                {/* Live visual Credit Card Preview */}
                <div className="relative overflow-hidden aspect-[1.8/1] rounded-2xl bg-linear-to-br from-zinc-800 via-zinc-900 to-black p-5 text-white flex flex-col justify-between shadow-lg select-none">
                  {/* Decorative card microchips */}
                  <div className="flex justify-between items-start">
                    <div className="w-10 h-8 rounded-md bg-linear-to-r from-amber-300 to-amber-500 opacity-90 relative flex items-center justify-center border border-amber-200/50 shrink-0">
                      <div className="w-8 h-6 border-r border-[#665e3b]/30" />
                    </div>
                    {/* Signal sensor */}
                    <div className="w-10 h-7 flex flex-col justify-between items-end opacity-20">
                      <div className="h-[2px] w-4 bg-white rounded" />
                      <div className="h-[2px] w-6 bg-white rounded" />
                      <div className="h-[2px] w-5 bg-white rounded" />
                    </div>
                  </div>

                  <div className="space-y-2">
                    {/* Digits row */}
                    <p className="font-mono text-sm sm:text-base tracking-widest text-center py-1">
                      {cardNumber ? cardNumber : "•••• •••• •••• ••••"}
                    </p>
                    
                    <div className="flex justify-between items-end">
                      <div className="min-w-0 flex-1 pr-4">
                        <span className="text-[8px] uppercase tracking-wider text-zinc-400 font-extrabold block">Cardholder</span>
                        <span className="text-xs font-mono font-bold uppercase truncate block text-zinc-100">
                          {cardName ? cardName : "ALEX REYNOLDS"}
                        </span>
                      </div>
                      <div className="shrink-0 flex gap-4">
                        <div>
                          <span className="text-[8px] uppercase tracking-wider text-zinc-400 font-extrabold block">Expires</span>
                          <span className="text-xs font-mono font-bold block text-zinc-100">
                            {cardExpiry ? cardExpiry : "MM/YY"}
                          </span>
                        </div>
                        <div>
                          <span className="text-[8px] uppercase tracking-wider text-zinc-400 font-extrabold block">CVC</span>
                          <span className="text-xs font-mono font-bold block text-zinc-100">
                            {cardCvc ? cardCvc : "•••"}
                          </span>
                        </div>
                ...
                {/* Form Inputs */}
                <form onSubmit={handleStripeSubmit} className="space-y-4 font-sans">
                  {paywallError && (
                    <div className="p-3 text-[11px] rounded-lg bg-red-50 border border-red-250 text-red-700 font-medium">
                      ⚠️ {paywallError}
                    </div>
                  )}
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="block text-[9px] font-black uppercase text-zinc-400 tracking-wider mb-1">Name on card</label>
                      <input
                        type="text"
                        required
                        ...
                        className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3 py-2.5 text-xs focus:outline-hidden focus:border-indigo-500 focus:bg-white text-zinc-800 font-medium transition-all"
                      />
                    </div>

                    <div>
                      <label className="block text-[9px] font-black uppercase text-zinc-400 tracking-wider mb-1">Card number</label>
                      <input
                        type="text"
                        required
                        ...
                        className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3 py-2.5 text-xs focus:outline-hidden focus:border-indigo-500 focus:bg-white text-zinc-800 font-medium transition-all font-mono"
                      />
                    </div>

                    <div className="grid grid-cols-3 gap-3">
                      <div className="col-span-1">
                        <label className="block text-[9px] font-black uppercase text-zinc-400 tracking-wider mb-1">Expiration</label>
                        <input
                          type="text"
                          required
                          placeholder="MM/YY"
                      ...
                      <div className="col-span-1">
                        <label className="block text-[9px] font-black uppercase text-zinc-400 tracking-wider mb-1">Security (CVC)</label>
                        <input
                          type="password"
                          required
                          placeholder="123"
                      ...
                      <div className="col-span-1">
                        <label className="block text-[9px] font-black uppercase text-zinc-400 tracking-wider mb-1">ZIP / Postal</label>
                        <input
                          type="text"
                          required
                          placeholder="90210"
                          value={stripeZip}
                          onChange={(e) => setStripeZip(e.target.value.slice(0, 10))}
                          className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3 py-2.5 text-xs focus:outline-hidden focus:border-indigo-500 focus:bg-white text-zinc-800 font-medium transition-all"
                        />
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full text-center py-3.5 bg-zinc-900 hover:bg-zinc-800 disabled:opacity-50 text-white font-bold rounded-xl text-xs transition-all shadow-md cursor-pointer flex items-center justify-center gap-2"
                  >
                    {paymentProcessing ? (
                      <>
                      ...
                      <>
                        <span>Activate Resources Now ($25.00)</span>
                      </>
                    )}
                  </button>
                </form>
              ...
              <div className="flex items-center gap-1.5 justify-center text-[10px] text-zinc-400 font-medium pt-3 mt-4 border-t border-zinc-150 border-dashed">
                <ShieldCheck className="w-3.5 h-3.5 text-zinc-400 shrink-0" />
                <span>Encrypted 256-bit Stripe checkout transaction. Syncs profile state instantly.</span>
              </div>
            </div>
      ...
      {user && !user.hasPaid && (
        <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-white px-8 py-2.5 text-xs font-semibold shrink-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 shadow-inner">
          <div className="flex items-center gap-2 flex-wrap text-zinc-50">
            ...
            <span>Educational sandbox trial: <strong>{user.trialCount} of 3</strong> previews used. Activate course status to synchronize database permanently.</span>
          </div>
          <button
            onClick={handleInstantBypass}
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        ...
        {showHighlightsNotebook && (
          <div className="w-full lg:w-80 border-t lg:border-t-0 lg:border-l border-zinc-200 bg-zinc-50/70 flex flex-col h-[350px] lg:h-full shrink-0 overflow-hidden animate-slideIn">
            ...
            {/* Notes Category Scope Switcher Tab Bar */}
            <div className="px-4 py-2 border-b border-zinc-155 bg-zinc-100 flex gap-1 shrink-0">
              <button
                onClick={() => setNoteScope('section')}
                className={cn(
                  "flex-1 text-center py-1 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all border cursor-pointer",
                  noteScope === 'section'
                    ? "bg-white border-zinc-200 text-indigo-600 shadow-xs"
                    : "bg-transparent border-transparent text-zinc-500 hover:text-zinc-800"
                )}
              >
                Section ({sectionHighlights.length})
              </button>
              <button
                onClick={() => setNoteScope('chapter')}
                className={cn(
                  "flex-1 text-center py-1 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all border cursor-pointer",
                  noteScope === 'chapter'
                    ? "bg-white border-zinc-200 text-indigo-600 shadow-xs"
              ...
              >
                Chapter ({chapterHighlights.length})
              </button>
            </div>
              ...
              {user && !user.hasPaid ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-4 py-8">
                  ...
                  <p className="text-xs font-semibold text-zinc-500 font-sans">
                    {noteScope === 'section' ? 'No Highlights in Section' : 'No Highlights in Chapter'}
                  </p>
                ...
                activeHighlightsList.map((hl) => {
                  ...
                  return (
                    ...
                    >
                      {/* Section identifier pill link */}
                      {noteScope === 'chapter' && SECTIONS_METADATA[hl.sectionId as SectionType] && (
                        <div className="flex items-center justify-between pointer-events-auto">
                          <button
                            onClick={() => onSelectChapterSection(chapter.id, hl.sectionId as SectionType)}
                            className="text-[8px] font-black uppercase tracking-widest text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 border border-indigo-150 rounded px-1.5 py-0.5 flex items-center gap-1 transition-all cursor-pointer"
                            title="Jump to this textbook section"
                          >
                            <BookOpen className="w-2.5 h-2.5" />
                            <span>{SECTIONS_METADATA[hl.sectionId as SectionType].title}</span>
                          </button>
                        </div>
                      )}
