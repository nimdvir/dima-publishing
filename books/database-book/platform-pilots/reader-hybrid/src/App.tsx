...
export default function App() {
  ...
  const goNext = useCallback(() => {
    if (scope === 'book' && currentPage) {
      const idx = FLAT_READER_PAGES.findIndex(p => p.id === currentPage.id);
      if (idx < FLAT_READER_PAGES.length - 1) {
        navigateToPage(FLAT_READER_PAGES[idx + 1]);
      }
    }
  }, [scope, currentPage, navigateToPage]);
  ...
  const hasNext = useMemo(() => {
    ...
    const idx = FLAT_READER_PAGES.findIndex(p => p.id === currentPage.id);
    return idx < FLAT_READER_PAGES.length - 1;
  }, [currentPage]);
  ...
  const nextPage = hasNext && currentPage
    ? FLAT_READER_PAGES[FLAT_READER_PAGES.findIndex(p => p.id === currentPage.id) + 1]
    : null;

  const prevPage = hasPrev && currentPage
    ? FLAT_READER_PAGES[FLAT_READER_PAGES.findIndex(p => p.id === currentPage.id) - 1]
    : null;
