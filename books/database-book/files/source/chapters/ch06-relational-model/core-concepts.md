...
## 1. What the Relational Model Does Differently

Before the relational model, databases used two main approaches: the **hierarchical model** and the **network model**. Both were navigational — to find data, application programs had to follow hard-coded paths through physical storage structures. If the database structure changed, programs broke. Data and programs were tightly coupled.

Edgar F. Codd, a researcher at IBM, saw the problem clearly. In his 1970 paper "A Relational Model of Data for Large Shared Data Banks," he proposed a radical alternative: separate the **logical view** of data (what users see) from the **physical storage** (how the DBMS organizes it internally). This principle of **data independence** meant that queries could ask *what* data was needed rather than specifying *how* to navigate through pointers to find it. Applications no longer broke when storage details changed.

This shift from procedural navigation to declarative querying — from *how* to *what* — is the foundation of SQL and of every modern relational database. The **relational model** organizes data into relations (tables) where each relation represents one well-defined subject and relationships are represented through keys. It is the discipline behind the flat-table fix.

| Characteristic | Hierarchical Model | Network Model | Relational Model |
|---|---|---|---|
| Data Structure | Tree (Parent-Child) | Graph (Owner-Member) | Tables (Relations) |
| Data Relationships | Pointers | Set Pointers | Values in Columns |
...
### 1.2 The Relational Model Is Not a Software Product
...
### 1.3 Properties of a Formal Relation
...
| Property                   | Meaning                                    | What Goes Wrong If Violated                                                 |
| -------------------------- | ------------------------------------------ | --------------------------------------------------------------------------- |
| One subject                | The table represents one clear subject     | Mixing students and grades causes modification anomalies (Section 1)        |
| Rows are instances         | Each row is one occurrence                 | Difficult to distinguish a "student" from a "grade event"                   |
| Columns are attributes     | Each column describes one property         | Ambiguity about what the data actually represents                           |
| Atomic values              | Each cell holds one indivisible value      | Storing `"Quiz 1, Quiz 2"` in one cell makes individual averages impossible |
| Consistent domain          | Values in a column are the same kind       | Text in a `Score` column breaks mathematical aggregations                   |
| Unique column names        | Each column has a distinct name            | Queries cannot distinguish between two columns named `Date`                 |
| Row order is irrelevant    | Meaning does not depend on row position    | Data becomes unreachable if the table is sorted or filtered                 |
...
<div class="callout key-takeaway">
  <p><strong>🔑 In short: Five rules for a true relation</strong></p>
  <ol>
    <li>Every cell must hold a single value.</li>
    <li>Every column must have a unique name.</li>
    <li>All values in a column must be of the same kind.</li>
    <li>The order of columns and rows does not matter.</li>
    <li>No two rows can be identical.</li>
  </ol>
  <p>These five rules are the minimum contract. A table that breaks any of them is not a true relation and will cause trouble when you try to query, join, or analyze it.</p>
</div>

### 1.4 Relational Vocabulary
...
### 1.5 Where This Chapter Is Going: The Seven-Table Schema
...
![Before-and-after comparison: one wide flat table on the left, seven connected relational tables on the right linked by keys.](https://res.cloudinary.com/dkndq6lyz/image/upload/f_auto,q_auto,c_limit,w_1200/Database-book-BITM330/ch06-relational-model/ch06-11-before-after-normalization)

_Figure 6.3 — Moving from one flat table to seven connected relational tables replaces redundancy with structure and integrity._

---

## 2. Entities, Attributes, Relationships, and Relations
...
![Four side-by-side cards labeled Entity, Attribute, Relationship, and Relation, each with a Grading Database example.](https://res.cloudinary.com/dkndq6lyz/image/upload/f_auto,q_auto,c_limit,w_1200/Database-book-BITM330/ch06-relational-model/ch06-10-relational-model-concept)

_Figure 6.4 — Entity, Attribute, Relationship, and Relation are the four vocabulary cornerstones of relational design._

---

## 3. Keys: How Tables Identify Rows

Keys are the foundation of relational identity. They let the database know exactly which row is being referenced.

### 3.1 Why Keys Are Necessary
...
### 3.2 Candidate and Primary Keys
...
_Figure 6.5 — Primary keys identify rows. Foreign keys connect tables. Candidate, natural, and surrogate keys serve different design roles._

---

## 4. Foreign Keys and Relationship Types
...
### 4.5 Relationship Types Summary
...
![Three side-by-side mini-diagrams illustrating one-to-one, one-to-many, and many-to-many relationships using Grading Database examples.](https://res.cloudinary.com/dkndq6lyz/image/upload/f_auto,q_auto,c_limit,w_1200/Database-book-BITM330/ch06-relational-model/ch06-05-relationship-types)

_Figure 6.6 — One-to-one, one-to-many, and many-to-many relationships each require a different structural representation in the schema._
...
## 5. Integrity Rules: Protecting Identity and Relationships
...
The same rule applies in any business domain. Consider a membership database:
...
The second payment is invalid because no member has `MemberID = 105`. You cannot record a payment from someone who does not exist in the system. And you cannot delete member 101 from `MEMBER` while payment 1 still references that member — the DBMS blocks the deletion to prevent an orphan payment. Referential integrity protects both directions: no orphan child records, and no deletion of a parent that is still in use.

This represents a fundamental shift in how data consistency is managed. In pre-relational systems, every application had to include its own logic to verify that a customer existed before creating an order. That approach was repetitive, inefficient, and error-prone. By defining referential integrity at the schema level, the database itself becomes an active guardian of its own consistency — enforced by the DBMS for every operation, regardless of which application or user initiates it. This centralization of business rules within the data layer is what makes relational databases more robust and trustworthy than the file-based systems they replaced.
...
### 5.4 Cascade Update and Cascade Delete
...
_Figure 6.7 — Referential integrity blocks orphan records: every non-null foreign key must match an existing primary key in the referenced table._

---

## 6. Redesigning the Grading Database
...
### 6.1 Stage-by-Stage Progression
...
![A full seven-table ERD for the Grading Database with crow's-foot notation and color-coded entity, lookup, and junction tables.](https://res.cloudinary.com/dkndq6lyz/image/upload/f_auto,q_auto,c_limit,w_1600/Database-book-BITM330/ch06-relational-model/ch06-07-grading-database-erd)

_Figure 6.8 — The full seven-table Grading Database ERD: each table represents one subject, and every relationship is enforced through keys._
...
## 7. Querying a Relational Design with Joins
...
### 7.3 Calculating Weighted Contribution
...
### 7.5 Advanced Preview: Finding Missing Grades
...
_Figure 6.9 — SQL joins reconstruct a report-ready view by reconnecting tables that relational design intentionally separated for integrity._

---

## 8. Microsoft Access as a Visual Learning Tool
...
### 8.3 Defining Relationships and Enforcing Integrity
...
### 8.4 Watching Referential Integrity Work
...
### 8.5 Inspecting the SQL Behind the Visual Query
...
## 9. Functional Dependencies and the Bridge to Normalization
...
![Functional dependencies: a bridge diagram showing how determinants map to their dependent attributes, previewing normalization.](https://res.cloudinary.com/dkndq6lyz/image/upload/f_auto,q_auto,c_limit,w_1200/Database-book-BITM330/ch06-relational-model/ch06-09-functional-dependency-bridge)

_Figure 6.10 — Functional dependencies reveal which attributes belong together because they share the same determinant, setting up the normalization work in Chapter 7._

<div class="callout key-takeaway">
  <p><strong>🔑 The enduring legacy of the relational model</strong></p>
  <p>Codd's 1970 paper transformed database management from an ad-hoc engineering task into a formal science grounded in set theory and predicate logic. The principles it established — uniqueness through keys, connections through foreign keys, consistency through referential integrity, and redundancy elimination through normalization — are not merely best practices. They are integral components of a cohesive, mathematically sound system. While NoSQL and other alternative models have emerged for specialized workloads, the relational model remains the bedrock for structured data worldwide, underpinning critical systems in finance, commerce, logistics, healthcare, and science. The skills you build in this chapter and the next transfer directly to every major relational database platform in use today.</p>
</div>
...
![Chapter 6 concept map: the relational model replaces flat-table redundancy with connected tables, keys, integrity rules, and the analytical power of joins.](https://res.cloudinary.com/dkndq6lyz/image/upload/f_auto,q_auto,c_limit,w_1200/Database-book-BITM330/ch06-relational-model/ch06-14-chapter-recap)

_Figure 6.11 — Chapter 6 recap: the relational model replaces flat-table redundancy with structure, integrity, and analytical power._
