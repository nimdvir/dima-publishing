...
<!-- Chapter edit (2026-06-17): fixed row count (six→ten), added Lab to GRADE_WEIGHT with explanatory note about INNER JOIN behavior. -->
---
...
This statement inserts ten rows at once in SQLite and PostgreSQL-style syntax.
...
This table now stores the grading structure needed for later joins and weighted calculations. Notice that `Lab` is included with a weight of zero — the lab is a separate graded component that contributes to the course outside the weighted exam/quiz/homework/project system. Row 10 in `GRADEBOOK` (Emma's Lab) has a `NULL` score and a `DeliverableType` of `Lab`. Because `GRADE_WEIGHT` now includes `Lab`, an `INNER JOIN` on `DeliverableType` will include that row. If `Lab` were absent from `GRADE_WEIGHT`, an `INNER JOIN` would silently drop Emma's Lab row — a preview of why Chapter 6 covers referential integrity and why matching values matter.
...
## Applying These Concepts to the Grading Database
