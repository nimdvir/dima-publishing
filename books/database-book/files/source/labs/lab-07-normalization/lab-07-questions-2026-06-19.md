...
- Builds on: the Chapter 7 Let's Build (normalizing the Grading Database together).
...
>
> **Normal forms at a glance:** Step 2 diagnoses the flat-table problems (Chapter 7, §7.1). Steps 3–9 split the flat table into six normalized tables — 1NF removes multi-valued cells and repeating columns (§7.4), 2NF separates facts that depend on only part of a composite key (§7.5), and 3NF moves facts that depend on a non-key column into their own table (§7.6). Step 10 enforces referential integrity so the connections stay valid (§7.7). Step 12 rebuilds the report, proving that normalization does not destroy your ability to see the data — it just stores each fact once (§7.8).
...
<div class="callout warning">
  <p><strong>⚠️ Warning: Close every open table first</strong></p>
  <p>Access will not let you edit relationships while a child table is open. Close every table before opening the Relationships window.</p>
</div>
