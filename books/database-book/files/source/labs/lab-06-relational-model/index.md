...
In this lab you take a starter PetVax database that already contains six tables of clinic data and turn it into a real relational design. You will identify primary keys, create five relationships with referential integrity enforced, prove that referential integrity is doing real work by trying to insert bad rows, and then build three saved queries that pay off the relational design â€” a multi-table billing join, a `LEFT JOIN` summary that surfaces an owner with no pets, and a missing-records anti-join that uses `CROSS JOIN` plus `LEFT JOIN ... IS NULL`.

- Chapter: Chapter 6 â€” The Relational Model.
...
PetVax is a small veterinary clinic. Until now its records have lived in a flat appointment spreadsheet. The owner has asked you to convert that into a real relational database so the clinic can ask harder questions â€” *who never came back for a vaccination?*, *which owners have no pets on file?*, *which appointments were billed off the standard fee?* â€” without scrolling through rows.
...
> âš ï¸ **Heads-up:** The starter file ships with **no relationships and no saved queries**. That is intentional. Building those is the graded work.
...
| Submission file 2 | `Lab06-PetVax-Relationships-LastName.pdf` (required â€” relationship report) |
...
### Step 1 â€” Open and save a working copy

**Do.** Download `lab-06-petvax-relational-starter.accdb` from `assets/`. Open it in Access. Use **File â†’ Save As â†’ Save Database As â†’ Access Database** and save the working copy as `Lab06-PetVax-Relational-LastName.accdb`. Close the starter file. From this point on, work only in your working copy.
...
### Step 2 â€” Inspect tables and primary keys

**Do.** Open each table in **Design View** (right-click the table name â†’ **Design View**). Look for the small key icon next to one field â€” that field is the primary key. Note down the primary key for each table. Then close every table before moving on.

**Check 2.** *(Matching)* Match each table to its primary key.

| Table | Primary Key options |
| --- | --- |
| `OWNER` | `OwnerID` Â· `OwnerName` Â· `OwnerEmail` |
| `PET` | `PetID` Â· `PetName` Â· `BreedName` |
| `PET_OWNER` | `PetOwnerID` Â· `PetID` Â· `OwnerID` |
| `APPOINTMENT` | `AppointmentID` Â· `PetID` Â· `AppointmentDate` |
| `SERVICE_TYPE` | `ServiceType` Â· `StandardFee` Â· `RequiresVaccineTracking` |
| `APPOINTMENT_SERVICE` | `AppointmentServiceID` Â· `AppointmentID` Â· `ServiceType` |
...
### Step 3 â€” Create the five relationships
...
**Do.** Open **Database Tools â†’ Relationships**. Click **Show Table** and add all six PetVax tables to the layout. Drag the parent field onto the matching child field to create each of the five relationships in the table below. For **every** relationship, in the **Edit Relationships** dialog:
...
| - | --- | --- | --- |
| 1 | `OWNER` | `PET_OWNER` | `OwnerID` â†’ `OwnerID` |
| 2 | `PET` | `PET_OWNER` | `PetID` â†’ `PetID` |
| 3 | `PET` | `APPOINTMENT` | `PetID` â†’ `PetID` |
| 4 | `APPOINTMENT` | `APPOINTMENT_SERVICE` | `AppointmentID` â†’ `AppointmentID` |
| 5 | `SERVICE_TYPE` | `APPOINTMENT_SERVICE` | `ServiceType` â†’ `ServiceType` |

Save the layout (**Ctrl+S**) and keep the Relationships window open for the next check.

:::callout{type="warning" title="âš ï¸ Do not check Cascade Delete"}
...
### Step 4 â€” Test referential integrity

Three invalid inserts that should all be rejected. For each one, open the target table in Datasheet View, type the bad row at the bottom (including the primary-key value shown â€” the PKs are manually entered, not AutoNumber), press Enter, read the error message Access shows, then press **Esc** to cancel the row. Do not leave any of these test rows in the table.
...
### Step 5 â€” Query 1: Appointment billing detail
...
**Do.** Open **Create â†’ Query Design**. Add four tables: `PET`, `APPOINTMENT`, `APPOINTMENT_SERVICE`, and `SERVICE_TYPE`. Access draws the join lines automatically from the relationships you defined in Step 3.
...
### Step 6 â€” Query 2: Owner pet summary with LEFT JOIN

This is the PetVax version of the Let's Build's attendance summary. The `LEFT JOIN` matters because it keeps every owner in the result â€” even an owner who has no pets on file.

**Do.** Open **Create â†’ Query Design**. Add `OWNER` and `PET_OWNER`. Right-click the join line between them, choose **Join Properties**, and pick option 2: *Include ALL records from `OWNER` and only those records from `PET_OWNER` where the joined fields are equal.* That is a `LEFT JOIN`.
...
Click the **Totals** (Î£) button on the ribbon. Set the first two fields to **Group By**. For `PetID`, change the cell from `Group By` to **Count**. Then rename the column by editing the Field row (top row) of the `PetID` column so it reads exactly:
...
:::callout{type="tip" title="ðŸ’¡ Stuck in Design View? Use SQL View instead"}
If the Totals row or alias is fighting you, switch the query to **SQL View** (Home â†’ View â†’ SQL View) and paste this in place of whatever is there:
...
:::callout{type="note" title="ðŸ“ Why Count(PetID) and not Count(*)"}
...
### Step 7 â€” Query 3: Find pets missing a required vaccination (CROSS JOIN + anti-join, stretch)

> **Stretch query â€” read slowly.** This mirrors the missing-grades query from the Let's Build. It is the most advanced query in the lab. You will build it in **two stages** so each move is easy to inspect.
...
1. **Enumerate the universe with `CROSS JOIN`.** `PET` (7 pets) Ã— `SERVICE_TYPE` (4 services) = 28 expected pet-service pairs. Filter to only the services that require vaccine tracking â€” there is one â€” and you get 7 expected pairs, one per pet. *(This is the first saved query.)*
...
#### Stage A â€” Build the universe

**Do.** Open **Create â†’ Query Design** â†’ **Close** the Show Table dialog â†’ switch to **SQL View** and paste:
...
Run it. You should see **7 rows** â€” every pet paired with `Vaccination`. Save as `qryExpectedRequiredVaccinations`.

The `FROM PET, SERVICE_TYPE` comma is Access's way of writing a cross join. Standard SQL and SQLite use the explicit keyword `CROSS JOIN` â€” same result.

#### Stage B â€” Anti-join against the universe
...
- C. PetID 3 â€” Coco
- D. PetID 4 â€” Charlie
- E. PetID 5 â€” Whiskers
- F. PetID 6 â€” Luna
- G. PetID 7 â€” Maple

### Step 8 â€” Export the relationship report

**Do.** Open **Database Tools â†’ Relationships** to bring the layout back into focus. On the ribbon, click **Relationship Report**. Access opens a print preview of the layout. Use **File â†’ Save As â†’ Save Object As â†’ PDF or XPS** (or **External Data â†’ PDF or XPS**) to export it as `Lab06-PetVax-Relationships-LastName.pdf` in the same folder as your `.accdb`.
...
1. `Lab06-PetVax-Relational-LastName.accdb` â€” your completed database with 5 relationships and 3 saved queries.
2. `Lab06-PetVax-Relationships-LastName.pdf` â€” the relationship report you exported in Step 8.
