...
-- Chapter 5
-- Submission file: Lab05-PetVax-SQL-LastName.sql
...
-- Check 1: Starter appointment count
SELECT COUNT(*) AS AppointmentCount
FROM PETVAX_APPOINTMENTS;

SELECT COUNT(*) AS ServiceRateCount
FROM SERVICE_RATES;

-- Check 3: No-show appointments
...
-- Check 4: Vaccine-due appointments
SELECT PetName, OwnerName, ServiceType, AppointmentDate
FROM PETVAX_APPOINTMENTS
WHERE VaccineDue = 'Yes'
ORDER BY AppointmentDate, AppointmentID;

SELECT COUNT(*) AS VaccineDueCount
FROM PETVAX_APPOINTMENTS
WHERE VaccineDue = 'Yes';

-- Check 5: Reminder not sent
SELECT PetName, AppointmentDate, AppointmentStatus, ReminderSent
FROM PETVAX_APPOINTMENTS
WHERE ReminderSent = 'No'
ORDER BY AppointmentDate, AppointmentID;

SELECT COUNT(*) AS ReminderNotSentCount
FROM PETVAX_APPOINTMENTS
WHERE ReminderSent = 'No';

-- Check 6: Distinct starter service types
SELECT DISTINCT ServiceType
FROM PETVAX_APPOINTMENTS
ORDER BY ServiceType;

SELECT COUNT(DISTINCT ServiceType) AS DistinctServiceTypeCount
FROM PETVAX_APPOINTMENTS;

-- Check 7: Highest starter payment
SELECT PetName, ServiceType, PaymentAmount
FROM PETVAX_APPOINTMENTS
ORDER BY PaymentAmount DESC, AppointmentID DESC
LIMIT 1;

-- Check 8: Add Zeus and Tofu, then count rows
INSERT INTO PETVAX_APPOINTMENTS
    (AppointmentID, AppointmentDate, AppointmentTime, PetName, OwnerName, OwnerEmail,
     AnimalType, BreedName, WeightKg, ServiceType, VaccineDue, AppointmentStatus,
     ReminderSent, PaymentAmount, Notes)
VALUES
    (21, '2026-04-23', '10:00 AM', 'Zeus', 'Adrian Voss', 'adrian.voss@example.com',
     'Dog', 'Great Dane', 55, 'Dental', 'No', 'Completed', 'Yes', 220,
     'First Dental of the month');
...
INSERT INTO PETVAX_APPOINTMENTS
    (AppointmentID, AppointmentDate, AppointmentTime, PetName, OwnerName, OwnerEmail,
     ...
     ReminderSent, PaymentAmount, Notes)
VALUES
    (22, '2026-04-24', '2:00 PM', 'Tofu', 'Hannah Reyes', NULL,
     'Cat', 'Sphynx', 4, 'Checkup', 'No', 'Completed', 'Yes', 80,
     'Owner email missing on file');

SELECT COUNT(*) AS AppointmentCountAfterInserts
FROM PETVAX_APPOINTMENTS;

-- Check 9: Missing owner email
SELECT AppointmentID, PetName, OwnerName, OwnerEmail
FROM PETVAX_APPOINTMENTS
WHERE OwnerEmail IS NULL;

SELECT COUNT(*) AS MissingOwnerEmailCount
FROM PETVAX_APPOINTMENTS
WHERE OwnerEmail IS NULL;

-- Check 10: Highest payment after inserts
SELECT PetName, ServiceType, PaymentAmount
FROM PETVAX_APPOINTMENTS
ORDER BY PaymentAmount DESC, AppointmentID DESC
LIMIT 1;

-- Check 11: Payments at least 100
SELECT PetName, ServiceType, PaymentAmount
FROM PETVAX_APPOINTMENTS
WHERE PaymentAmount >= 100
ORDER BY PaymentAmount DESC, AppointmentID DESC;

SELECT COUNT(*) AS PaymentsAtLeast100Count
FROM PETVAX_APPOINTMENTS
WHERE PaymentAmount >= 100;

-- Check 12: Service types missing from SERVICE_RATES
SELECT a.AppointmentID,
       a.PetName,
       a.ServiceType,
       a.PaymentAmount,
       r.StandardFee
FROM PETVAX_APPOINTMENTS AS a
LEFT JOIN SERVICE_RATES AS r
       ON a.ServiceType = r.ServiceType
WHERE r.StandardFee IS NULL
ORDER BY a.AppointmentID;

-- Check 13: Average payment by matched service, filtered with HAVING
SELECT r.ServiceType,
       ROUND(AVG(a.PaymentAmount), 2) AS AvgPayment
FROM PETVAX_APPOINTMENTS AS a
INNER JOIN SERVICE_RATES AS r
        ON a.ServiceType = r.ServiceType
GROUP BY r.ServiceType
HAVING AVG(a.PaymentAmount) >= 100
ORDER BY r.ServiceType;

-- Check 14: Non-zero payment differences
SELECT a.AppointmentID,
       a.PetName,
       a.ServiceType,
       a.PaymentAmount,
       r.StandardFee,
       (a.PaymentAmount - r.StandardFee) AS PaymentDifference
FROM PETVAX_APPOINTMENTS AS a
INNER JOIN SERVICE_RATES AS r
        ON a.ServiceType = r.ServiceType
WHERE a.PaymentAmount <> r.StandardFee
ORDER BY a.AppointmentID;

SELECT COUNT(*) AS PaymentMismatchCount
FROM PETVAX_APPOINTMENTS AS a
INNER JOIN SERVICE_RATES AS r
        ON a.ServiceType = r.ServiceType
WHERE a.PaymentAmount <> r.StandardFee;

-- Step 15: Optional safe-update preview only
SELECT AppointmentID, PetName, ServiceType, PaymentAmount
FROM PETVAX_APPOINTMENTS
WHERE AppointmentID = 5;

-- Do not run unless instructed.
-- UPDATE PETVAX_APPOINTMENTS
-- SET    PaymentAmount = 65
-- WHERE  AppointmentID = 5;
