...
## Detailed Book Outline — June 12, 2026

> Derived from actual chapter headings (H2/H3) in the most recent main `.md` files as of June 12, 2026. Chapters 9 and 10 have been renumbered (Design first, then Advanced SQL) to match the pedagogical arc.
...
- **Why the Book Is Sequenced This Way** — Explains the deliberate chapter progression from business value through design, queries, analytics, and strategy, with a chapter-by-chapter table mapping topic focus to cognitive goals.
...
- **SQL and Platforms as the Next Step** — Previews Chapter 5 by introducing SQL as the standard language for working with databases, and surveys the platforms used in the course: Microsoft Access, SQLite, and Supabase/PostgreSQL.
  - *Microsoft Access as Your First Database Environment* — Introduces Access as the visual learning platform for the course; explains the four core objects (tables, queries, forms, reports), Datasheet View vs Design View, the Query by Example (QBE) editor, and the Access-vs-Excel distinction.
- **Getting Hands-On with Microsoft Access** — A practical walkthrough building a complete database from scratch using the GymMembership scenario.
  - *The Anatomy of an Access Database: Core Objects* — Tables, queries, forms, and reports as the four object types; navigation pane, ribbon, and work area overview.
  - *Your Workspace: Datasheet View and Design View* — Contrasts the data-entry grid with the structure-definition view; includes a complete Access data types reference table (Short Text, Long Text, Number, Date/Time, Currency, Yes/No, AutoNumber) and the "looks numeric → Short Text" rule for identifiers.
  - *Building a Database from Scratch* — Step-by-step GymMembership database creation: blank database, Members table (MemberID AutoNumber PK, FirstName, LastName, JoinDate), Payments table (PaymentID AutoNumber PK, MemberID Number FK, PaymentDate, Amount), establishing relationships with referential integrity enforced.
  - *Interacting with Your Data* — Sorting (ascending/descending by column), filtering (Selection Filter, Filter by Form), data entry/edit/delete mechanics, and essential maintenance utilities (Back Up Database, Compact and Repair).
  - *From This Chapter to the Let's Build* — Bridges the Access hands-on section to the Let's Build companion and previews the SQL work in Chapter 5.
- **Summary** — Condensed chapter takeaways.
- **Figures Index** — List of all figures appearing in the chapter.
...
- **9.1 From Querying Data to Designing Systems** — The role shift from database user to database designer.
  - *9.1.1 Good Queries Require Good Design* — Many "query problems" are design problems in disguise.
  - *9.1.2 The Shift from User to Designer* — New questions: what must the system remember, which objects deserve tables, which rules should the DBMS enforce?
  - *9.1.3 Design as Translation* — Mapping business language to data structure with concrete examples.
- **9.2 The Cost of Poor Design: Data Anomalies** — Why design quality matters before implementation.
  - *9.2.1 Starting Example: A Flat Grading Table* — A concrete poorly designed table.
  - *9.2.2 Insertion Anomaly* — Cannot add one fact without unrelated facts.
  - *9.2.3 Update Anomaly* — Changing one fact requires many row changes.
  - *9.2.4 Deletion Anomaly* — Deleting one fact accidentally destroys another.
  - *9.2.5 Why Anomalies Matter* — Anomalies damage trust, increase labor, and weaken decisions.
- **9.3 Database Design in the System Development Life Cycle** — Where database design fits in system development.
  - *9.3.1 What Is the SDLC?* — Planning, building, deploying, and maintaining systems through deliberate phases.
  - *9.3.2 SDLC Phases from a Database Perspective* — Planning, conceptual design, logical design, physical design, development, testing, deployment, maintenance.
  - *9.3.3 Conceptual, Logical, and Physical Design* — Three design levels: what the domain contains, what tables are needed, how a specific DBMS implements them.
- **9.4 From Requirements to Structure** — Translating business needs into design elements.
  - *9.4.1 Requirements as Design Inputs* — How stated requirements imply entities, attributes, and relationships.
  - *9.4.2 Entities* — Real-world objects, concepts, or events the database must represent.
  - *9.4.3 Attributes* — Properties or characteristics of entities.
  - *9.4.4 Relationships* — Associations between entities.
  - *9.4.5 Business Rules* — Constraints the database must enforce.
- **9.5 Entity-Relationship Modeling** — The visual language of database design.
  - *9.5.1 What ER Modeling Is* — A graphical method for representing entities, attributes, and relationships.
  - *9.5.2 ERD Elements* — Rectangles (entities), ovals (attributes), diamonds (relationships), lines (connections).
  - *9.5.3 Key Hierarchy* — Primary keys, foreign keys, and candidate keys in ERDs.
  - *9.5.4 Example: STUDENT and STUDENT_GRADE* — Concrete ERD walkthrough.
- **9.6 Crow's Foot Notation** — The industry-standard notation for cardinality and optionality.
  - *9.6.1 What Crow's Foot Notation Shows* — Minimum and maximum cardinality visually.
  - *9.6.2 Reading Crow's Foot Notation* — Interpreting the symbols: zero, one, many.
  - *9.6.3 Crow's Foot to SQL* — How cardinality translates to foreign keys and constraints.
- **9.7 Understanding Relationship Types** — The three fundamental relationship patterns.
  - *9.7.1 One-to-One Relationships* — Rare but important: one row in A matches at most one row in B.
  - *9.7.2 One-to-Many Relationships* — The most common pattern.
  - *9.7.3 Many-to-Many Relationships* — Requires a junction/associative table to resolve.
  - *9.7.4 Relationships in the Grading Database* — Mapping the actual relationships in the course database.
- **9.8 Advanced ER Modeling Concepts** — Beyond the basics.
  - *9.8.1 Weak Entities* — Entities whose existence depends on another entity.
  - *9.8.2 Associative Entities* — Entities that resolve M:N relationships and carry their own attributes.
  - *9.8.3 Recursive Relationships* — An entity related to itself (e.g., employee-manager).
  - *9.8.4 Specialization and Generalization* — Subtype/supertype hierarchies.
  - *9.8.5 Mapping Specialization to Tables* — Design options for implementing inheritance in relational tables.
- **9.9 Normalization as a Design-Quality Check** — Using normal forms to validate a design.
  - *9.9.1 Normal Forms Review* — 1NF, 2NF, 3NF as progressive design tests.
  - *9.9.2 Applying Normalization to Design* — Checking a proposed schema against normal forms.
  - *9.9.3 When to Denormalize* — Intentional redundancy and its trade-offs.
- **9.10 From ER Diagrams to Relational Tables** — The mapping algorithm.
  - *9.10.1 Step 1: Map Strong Entities* — Each strong entity becomes a table; attributes become columns.
  - *9.10.2 Step 2: Map Weak Entities* — Include the owner's primary key as part of the weak entity's key.
  - *9.10.3 Step 3: Map 1:N Relationships* — Add a foreign key to the "many" side.
  - *9.10.4 Step 4: Map M:N Relationships* — Create a new junction table with composite primary key.
  - *9.10.5 Step 5: Map Special Attributes* — Multi-valued attributes and derived attributes.
  - *9.10.6 Complete Grading Database ERD in Mermaid* — The full design rendered as Mermaid.js ERD code.
- **9.11 Visual Schema Design Tools** — Tools for creating and sharing ERDs.
  - *9.11.1 Lucidchart* — Visual drag-and-drop ERD tool with collaboration features.
  - *9.11.2 Mermaid* — Text-based ERD-as-code for version-controlled documentation.
  - *9.11.3 Lucidchart vs. Mermaid* — Trade-offs: visual fidelity vs. maintainability and version control.
- **9.12 Common Database Modeling Mistakes** — Pitfalls to avoid.
  - *9.12.1 Building Before Modeling* — Jumping to implementation without design.
  - *9.12.2 Treating Reports as Tables* — Confusing output format with storage structure.
  - *9.12.3 Failing to Resolve Many-to-Many Relationships* — Leaving M:N unresolved.
  - *9.12.4 Putting Foreign Keys on the Wrong Side* — Reversing the FK direction.
  - *9.12.5 Omitting Optionality* — Not specifying whether a relationship is required or optional.
  - *9.12.6 Confusing Attributes with Entities* — When a property should be its own table.
  - *9.12.7 Storing Derived Values Too Early* — Calculated fields that become stale.
  - *9.12.8 Ignoring Naming Conventions* — Inconsistent names that confuse future users.
- **9.13 Design vs. Implementation** — The critical distinction.
  - *9.13.1 Logical Design Is Platform-Independent* — Tables, keys, and relationships defined without reference to a specific DBMS.
  - *9.13.2 Physical Design Is Platform-Specific* — Data types, indexes, and storage tuned to a particular DBMS.
  - *9.13.3 Why the Distinction Matters* — The tool should follow the model, not drive it.
- **9.14 Strengths and Limits of ER Modeling** — Honest assessment of the technique.
  - *9.14.1 Strengths* — Visual, standardized, bridges business and technical audiences.
  - *9.14.2 Limits* — Does not capture behavior, workflows, or non-relational data well.
  - *9.14.3 Alternatives and Complements* — UML, data flow diagrams, dimensional modeling.
- **9.15 Key Concepts** — Bulleted recap of essential terms.
- **9.16 Chapter Summary** — Condensed chapter takeaways.
- **9.17 Review Questions** — Check-understanding questions.
- **9.18 References** — Cited works and further reading.
...
Builds on the design foundations of Chapter 9 to take SQL from basic queries to sophisticated analysis. Covers complex JOINs, nested subqueries, window functions, CASE expressions, CTEs, grade analytics, data diagnostics, weighted grades, date queries, safe data modification, and reusable reporting pipelines.

- **Learning Objectives** — What students will be able to do after completing this chapter.
- **Chapter Roadmap** — Table mapping each section to its main question and core idea.
- **10.1 From Basic SQL to Advanced SQL** — Reframes SQL as a tool for answering increasingly sophisticated business questions.
...
- **10.2 Grading Database Refresher** — Re-establishes the full schema before diving into advanced queries.
...
- **10.3 Diagnosing Data Problems with SQL** — Using SQL to find what is wrong before analysis begins.
  - *10.3.1 Detecting Repeated Student Details in a Flat Table* — Finding duplicate identity data.
  - *10.3.2 Detecting Conflicting Emails* — Finding inconsistent copies of the same fact.
  - *10.3.3 Detecting Inconsistent Deliverable Definitions* — Finding deliverable metadata that varies by row.
  - *10.3.4 Detecting Duplicate Grade Records* — Finding multiple scores for the same student-deliverable pair.
  - *10.3.5 Detecting Scores Outside the Valid Range* — Finding out-of-bounds numeric data.
  - *10.3.6 Detecting Orphaned Grade Records* — Finding grades that reference nonexistent students or deliverables.
- **10.4 Advanced JOIN Patterns** — Multi-table joins for real business reports.
  - *10.4.1 Complete Gradebook Report* — Joining STUDENT, DELIVERABLE, and STUDENT_GRADE for a full roster view.
  - *10.4.2 Adding Assignment-Type Rules* — Joining through ASSIGNMENT_TYPE for weighted-category logic.
  - *10.4.3 Finding Missing Grades with `CROSS JOIN` and `LEFT JOIN`* — Detecting student-deliverable combinations with no score.
  - *10.4.4 Finding Deliverables with No Submissions* — Identifying deliverables no student has attempted.
  - *10.4.5 Joining Attendance to the Schedule* — Combining STUDENT, SCHEDULE, and ATTENDANCE for attendance tracking.
  - *10.4.6 Join Checklist* — Decision tree for choosing the right join type.
- **10.5 Cleaning and Conditional Functions** — Handling missing, inconsistent, or ambiguous data.
  - *10.5.1 Access SQL and Portable SQL Compared* — Platform differences in conditional functions.
  - *10.5.2 Replacing Missing Scores for Display* — COALESCE and NZ for NULL substitution.
  - *10.5.3 Conditional Labels with `IIf()` and `CASE`* — Assigning performance labels based on score thresholds.
  - *10.5.4 Multi-Level Performance Bands* — Nested CASE for letter-grade or band assignment.
  - *10.5.5 Cleaning Text Values* — TRIM, UPPER, LOWER, and string-manipulation patterns.
- **10.6 Analytical Aggregation** — Summarizing data for insight.
  - *10.6.1 Average Score Per Student* — Per-student performance metrics.
  - *10.6.2 Average Score Per Deliverable* — Per-deliverable difficulty analysis.
  - *10.6.3 Assignment-Type Averages* — Category-level performance comparisons.
  - *10.6.4 `HAVING` for Group Filters* — Filtering groups after aggregation.
  - *10.6.5 Conditional Aggregation* — COUNT with CASE for counting specific conditions.
  - *10.6.6 Score Bands* — Bucketing scores into performance tiers.
  - *10.6.7 Attendance Rate Per Student* — Computing presence percentages.
  - *10.6.8 `COUNT(*)` vs. `COUNT(column)`* — The critical distinction and when each matters.
- **10.7 Date and Time Queries** — Temporal analysis patterns.
  - *10.7.1 Date Function Reference* — Quick-reference for date functions across platforms.
  - *10.7.2 Upcoming Deliverables* — Filtering by future due dates.
  - *10.7.3 Overdue Deliverables Without Grades* — Finding missing work past deadline.
  - *10.7.4 Days Since Due Date* — Computing elapsed time.
  - *10.7.5 Attendance Trend by Week* — Aggregating attendance over time periods.
- **10.8 Weighted Grades and Policy Tables** — Computing final grades from policy rules.
  - *10.8.1 Example Weight Table* — The ASSIGNMENT_TYPE table as policy storage.
  - *10.8.2 Average Score by Student and Category* — Per-category performance.
  - *10.8.3 Weighted Final Grade Using a CTE* — Multi-step weighted computation.
  - *10.8.4 Handling Missing Categories* — What happens when a student has no grades in a category.
  - *10.8.5 Joining to the Grade Scale* — Converting numeric weighted grades to letter grades.
- **10.9 Window Functions** — Analytics without collapsing rows.
  - *10.9.1 `GROUP BY` vs. Window Functions* — When to use each.
  - *10.9.2 Student Average Next to Each Score* — OVER() for per-row context.
  - *10.9.3 Ranking Students by Average* — RANK() and ROW_NUMBER() for ordering.
  - *10.9.4 Running Total by Student* — Cumulative sums with window frames.
  - *10.9.5 Moving Average* — Rolling-window calculations.
  - *Platform Note* — Window function availability across Access, SQLite, and PostgreSQL.
- **10.10 Reusable Reporting Pipelines** — Building persistent analytical structures.
  - *10.10.1 Views as Saved Reports* — CREATE VIEW for reusable query definitions.
  - *10.10.2 CTEs for Multi-Step Logic* — WITH clauses for readable, step-by-step query construction.
  - *10.10.3 Subqueries for One-Step Comparisons* — Nested SELECT for inline calculations.
  - *10.10.4 `EXISTS` for Relationship Checks* — Testing for the presence of related rows.
  - *10.10.5 `UNION` and `UNION ALL`* — Combining result sets vertically.
  - *10.10.6 Choosing the Right Tool* — Decision guide: view vs. CTE vs. subquery vs. EXISTS.
- **10.11 Safe Data Modification** — Writing UPDATE and DELETE that do not destroy data.
  - *10.11.1 Safe `UPDATE`* — Always test with SELECT first; always use WHERE.
  - *10.11.2 Updating Scores* — Correcting grade entries with targeted UPDATE.
  - *10.11.3 Safe `DELETE`* — Verifying what will be removed before executing.
  - *10.11.4 Dangerous Patterns* — UPDATE without WHERE, DELETE without WHERE, ambiguous joins in UPDATE.
  - *10.11.5 Transactions as a Safety Preview* — BEGIN/ROLLBACK/COMMIT as an undo safety net.
- **10.12 Integrated Example: At-Risk Student Report** — End-to-end capstone query combining multiple advanced techniques.
...
- **13.6 Window Functions: Analytics Without Losing Detail** — Deeper treatment following Chapter 10's introduction.
