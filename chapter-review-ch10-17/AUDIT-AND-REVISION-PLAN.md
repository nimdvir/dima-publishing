...
### A1. Missing chapter overview videos
Every chapter 11–17 has a placeholder instead of a video. Only ch10 has a real embed.
- Affected `index.md` files: **ch11:9, ch12:7, ch13:7, ch14:9, ch15:7, ch16:7, ch17:9**
- Each contains: `> **Video placeholder:** Chapter N overview video will be added here before publication.`
- **Fix:** replace each placeholder with the ch10 pattern (needs the real YouTube ID per chapter):
  ```html
  ...
  ```
  Reference: `ch10-advanced-sql-queries/index.md` lines 7–11.
  **Blocker:** YouTube IDs for ch11–17 are needed (only ch10's `kFlSsAMlYTU` exists). If videos
  don't exist yet, the placeholders should stay but be styled consistently.

### A2. Broken / incomplete "Chapter Roadmap" tables (= "On this page is incorrect")
The roadmap tables in `index.md` are inconsistent and, in several chapters, start mid-chapter or
...
5. **A4 figures** (ch11 placeholders + others) and **A1 videos** — both need external assets
   (Cloudinary images, YouTube IDs) and should be done once those are supplied.
